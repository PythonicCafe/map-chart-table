 ...
 * @property {string} [label] - Sigla/Label
 * @property {string} [color] - Cor Hex ou RGB
 * @property {number} [population] - População (usado no tooltip)
 * @property {number} [doses] - Doses (usado no tooltip)
 * @property {string|number} [id] - ID opcional
 */

/**
 * @typedef {Object} MapChartOptions
 * @property {HTMLElement} element
 * @property {string} [map]
 * @property {Object.<string, DatasetItem>} [datasetCities]
 * @property {Array<Array<string|number>>} [cities]
 * @property {Object.<string, DatasetItem>} [datasetStates]
 * @property {Array<Array<string|number>>} [states]
 * @property {string[]} [statesSelected]
 * @property {function(boolean, string, string|number): void} [tooltipAction]
 * @property {string} [type]
 * @property {boolean} [formPopulated]
 * @property {boolean} [loading]
 */

export class MapChart {
    /**
     * @param {MapChartOptions} options
     */
    constructor({
        element,
        map,
        ...
        this.map = map
        /** @type {Object.<string, DatasetItem> | undefined} */
        this.datasetCities = datasetCities
        /** @type {Array<Array<string|number>> | undefined} */
        this.cities = cities
        /** @type {Object.<string, DatasetItem> | undefined} */
        ...
        /** @type {((opened: boolean, name: string, id: string|number) => void) | undefined} */
     ...
     * @returns {[number, number, number, (string|number)[] | undefined]}
     */
    getData(contentData, elementId) {
        ...
        const index =
            contentData[0].indexOf('geom_id') != -1
                ? contentData[0].indexOf('geom_id')
                : contentData[0].indexOf('id')
        ...
        return [
            ...
            indexAcronym,
            contentData.find(
                (/** @type {string[]} */ el) => el[index] === elementId
            ),
        ]
    }
    ...
    /**
     * @param {Object} params
     * @param {Array<any>} [params.datasetStates]
     * @param {Array<Array<string|number>>} [params.contentData]
     * @param {string} [params.type]
     */
    setData({ datasetStates, contentData, type } = {}) {
        const self = this

        // Querying map country states setting eventListener
        for (const element of self.element.querySelectorAll(
            '#canvas path, #canvas g'
        )) {
            let elementId = element.id

            if (elementId.length > 6) {
                // granularity Municipios
                elementId = element.id.substring(0, elementId.length - 1)
            }

            // @ts-ignore
            let [index, indexName, indexAcronym, currentElement] = self.getData(
                contentData,
                elementId
            )
            ...
            /** @type {DatasetItem} */
            let dataset = { value: '---', color: '#D3D3D3' }
            /** @type {any} */
            let datasetValuesFound = []

            // @ts-ignore: Acesso dinâmico a propriedade 'id' em array genérico
            if (content.id) {
                // Get by id
                // @ts-ignore
                datasetValuesFound = self.datasetValues.find(
                    (ds) =>
                        ds.name == content[indexName] &&
                        ds.id == content[indexName]
                )
            } else {
                // Get by name
                datasetValuesFound = self.datasetValues.find(
            ...
            // @ts-ignore
            const result = dataset.data || dataset
            const resultColor = dataset.color
            const tooltip = /** @type {HTMLElement} */ (
                self.element.querySelector('.mct-tooltip')
            )
          ...
          `
            ...
            `
                ...
                if (type === 'Meta atingida') {
                    if (result.value !== '---') {
                        // @ts-ignore: parseInt em string|number
          ...
          <article>
            ...
            ${tooltipExtra}
                ...
                // @ts-ignore: includes em number|string
                tooltip.style.backgroundColor = result.value
                    .toString()
                    .includes('---')
                    ? 'grey'
                    : 'var(--primary-color)'
            ...
            htmlElement.addEventListener('mouseleave', (event) => {
                const target = /** @type {HTMLElement} */ (event.target)
                if (target.tagName === 'g') {
                    ;[...target.querySelectorAll('path')].forEach(
                        (path) => (path.style.strokeWidth = '1px')
                    )
                } else {
                    htmlElement.style.strokeWidth = '1px'
                }

                htmlElement.style.fill = resultColor || ''
                htmlElement.style.stroke = 'white'
        ...
        // dataResult is undefined if nothing is comming from API and selects is setted
        if (
            self.formPopulated &&
            (!datasetStates || !datasetStates.length) &&
            !this.loading
        ) {
            const span = self.element.querySelector('.empty-message span')
            if (span) {
                span.innerHTML =
                    'Não existem dados para os filtros selecionados'
            }

            const emptyMessage = /** @type {HTMLElement} */ (
                self.element.querySelector('.empty-message')
            )
            if (emptyMessage) {
    ...
    findElement(arr, name) {
        for (let i = 0; i < arr.length; i++) {
            ...
            const nameAcronymLowerCase = name.acronym
    ...
    /**
     * @param {Object.<string, DatasetItem>} dataset
     */
    getMaxAndMinValues(dataset) {
        // @ts-ignore: includes em number|string
        if (Object.values(dataset)[0].value.toString().includes('%')) {
            return
        }

        const values = Object.values(dataset).map((val) =>
            val.value.toString().replace(/[,.]/g, '')
        )
        // @ts-ignore: Spread em string[]
    ...
    loadMapNation() {
        ...
        if (self.datasetStates) {
            ...
            result = Object.entries(self.datasetStates)
                .map(([key, val]) => {
                    let color = resultValues
                        ? self.getPercentage(
                              // @ts-ignore
                ...
                .filter((content) => {
                    if (content && self.statesSelected) {
                        // @ts-ignore
    ...
    getColor(percentage, maxVal = 100, type, reverse = false) {
        ...
        const upperColor =
            index < colors.length - 1 ? colors[index + 1] : colors[index]
        const factor = (percentage / maxVal) * (colors.length - 1) - index
        const interpolatedColor = {
            r: Math.round(
                lowerColor.r + (upperColor.r - lowerColor.r) * factor
            ),
            g: Math.round(
                lowerColor.g + (upperColor.g - lowerColor.g) * factor
            ),
            b: Math.round(
                lowerColor.b + (upperColor.b - lowerColor.b) * factor
            ),
        }
    ...
    render() {
        ...
        if (legendSvg) {
            legend = `<img class="mct-legend-svg" src=${legendSvg} alt="map file" />`
        }
    ...
    `
      ...
      <section>
        ...
        <div class="empty-message" style="position: absolute; top: 35%; width: 100%; text-align:center; color: #777; font-weight: 500; display:none;">
          ${emptyIcon}
