...
export default {
    ...
    UPDATE_FORM(state, payload) {
        ...
        if (
            ...
            const activeDoses = state.form.doses.filter(
                (dose) => !dose.disabled
            )
    ...
    UPDATE_TAB(state, payload) {
        ...
        if (['table', 'chart'].includes(Object.values(payload)[0])) {
            ...
            disableOptionsByDoseOrSick(state, {
                ['sickImmunizer']: state.form.sickImmunizer,
            })
            this.commit(
                'message/INFO',
                'Seletores atualizados para tipo de exibição selecionada',
    ...
    UPDATE_TABBY(state, payload) {
        ...
        state.form.sickImmunizer = Array.isArray(state.form.sickImmunizer)
            ? []
            : null
            ...
            if (key === 'form') {
                for (let [formKey, formValue] of Object.entries(value)) {
                    if (Array.isArray(state.form[formKey])) {
                        state.form[formKey] = state.form[formKey].concat(
                            ...state.form[formKey],
                            formValue
                        )
                    } else if (
                        formKey === 'sickImmunizer' &&
                        state.tab !== 'map'
                    ) {
                        state.form[formKey] = Array.isArray(formValue)
                            ? formValue
                            : [formValue]
                    } else {
