...
export default {
    ...
    async updateFormSelect({ commit, state }) {
        const api = new DataFetcher(state.apiUrl)
        const payload = {}
    ...
    async requestData(
        ...
        if (result.error) {
            ...
            commit(
                'UPDATE_CSV_MAX_EXPORT_ROWS',
                result.metadata.max_csv_export_rows
            )
        }
        ...
        if (form.type !== 'Doses aplicadas') {
            result.data
                .slice(1)
                .forEach((x, i) => (x[2] = Number(x[2]).toFixed(2) + '%'))
    ...
    async requestPage({ state, commit }, [mutation, slug]) {
        const api = new DataFetcher(state.apiUrl)
        const payload = await api.requestSettingApiEndPoint(
            slug,
            '/wp-json/wp/v2/pages'
        )
    ...
    async requestJson({ state, commit }, [mutation, endpoint]) {
        const api = new DataFetcher(state.apiUrl)
        try {
