        ...
        const App = {
            ...
            setup() {
                ...
                const tab = computed(
                    computedVar({
                        store,
                        mutation: 'content/UPDATE_TAB',
                        field: 'tab',
                    })
                )
                const tabBy = computed(
                    computedVar({
                        store,
                        mutation: 'content/UPDATE_TABBY',
                        field: 'tabBy',
                    })
                )
                const disableMap = computed(
                    () => store.state.content.disableMap
                )
                const disableChart = computed(
                    () => store.state.content.disableChart
                )
                ...
                const genericModalShow = computed(
                    computedVar({
                        store,
                ...
                const genericModalLoading = computed(
                    computedVar({
                        store,
                ...
                self.genericModal = async (title, slug) => {
                    ...
                    try {
                        await store.dispatch('content/requestPage', [
                            'UPDATE_GENERIC_MODAL',
                            slug,
                        ])
                    } catch {
                        // Do Nothing
                ...
                self.genericModalWithFilterButton = async (title, slug) => {
                    await store.dispatch('content/updateExtraFilterButton', [
                        title,
                        slug,
                    ])
                }
                ...
                onBeforeMount(async () => {
                    ...
                    await Promise.all([
                        [
                            ...
                            [
                                'UPDATE_MANDATORY_VACCINATIONS_YEARS',
                                'mandatory-vaccinations-years',
                            ],
                            ['UPDATE_LAST_UPDATE_DATE', 'lastupdatedate'],
                            ['UPDATE_AUTO_FILTERS', 'auto-filters'],
                            ['UPDATE_ACRONYMS', 'acronyms'],
                        ].map((request) =>
                            store.dispatch('content/requestJson', request)
                        ),
                        [
                            [
                                'UPDATE_ABOUT_VACCINES',
                                '?slug=sobre-vacinas-vacinabr',
                            ],
                        ].map((request) =>
                            store.dispatch('content/requestPage', request)
                        ),
                    ])
                })
                ...
                const modalContent = computed(() => {
                    ...
                    if (div.querySelector('table')) {
                        const result = [
                            ...div.querySelectorAll('table>tbody>tr'),
                        ].map((tr) => {
                            return {
                ...
                return {
                    ...
                    modalContent,
                    disableAll: computed(
                        () => store.state.content.yearSlideAnimation
                    ),
                    biMap,
                    biGraphUp,
                    biTable,
