...
export const mapRange = {
    ...
    setup(props) {
        ...
        const samePercentCircle = (svg, i, data, meta) =>
        ...
        const handleMapChange = (data) => {
            ...
            if (type === 'Doses aplicadas') {
                maxDataVal = Math.max(
                    ...data.map((x) => x.data.value.replace(/[.,]/g, ''))
                )
                defineMinVal = 0
        ...
        watch(
            () => props.mapTooltip,
            () => {
                const query = props.mapTooltip.label
                    ? `[data-id="${props.mapTooltip.label}"]`
                    : `[data-title="${props.mapTooltip.name}"]`
