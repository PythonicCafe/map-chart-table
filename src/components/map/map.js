...
export const map = {
    ...
    setup(props, { emit }) {
        ...
        const loading = computed(
            computedVar({
                store,
                mutation: 'content/UPDATE_LOADING',
                field: 'loading',
            })
        )
        ...
        const queryMap = async (local) => {
            ...
            if (
                granularity.value === 'Macrorregião de saúde' &&
                local.length > 1
            ) {
                maplocal = 'macreg/BR'
                ...
                granularity.value === 'Região de saúde' &&
                local.length > 1
            ) {
                maplocal = `reg/BR`
            ...
            const file = await store.dispatch(`content/requestMap`, {
                map: maplocal,
            })
            return file
        }
        ...
        const setMap = async () => {
            ...
            try {
                ...
                if (local.length === 1) {
                    datasetCities.value = convertArrayToObject(
                        results.data
                    ).data
                    datasetStates.value = null
                    updatePeriod()
                    ...
                    datasetCities.value = null
                    datasetStates.value = convertArrayToObject(
                        results.data
                    ).data
                    updatePeriod()
                    mapSetup = {
