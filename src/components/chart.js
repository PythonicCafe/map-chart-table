...
export const chart = {
    ...
    setup() {
        ...
        const splitTextToChart = (label) => {
            ...
            if (
                store.state.content.form.granularity.toLowerCase() ===
                'municípios'
            ) {
        ...
        const setChartData = async () => {
            ...
            for (let [key, value] of chartResultEntries) {
                if (i > 99) {
                    store.commit(
                        'message/INFO',
                        'Essa filtragem excedeu o máximo de 30 linhas, apenas 30 linhas serão exibidas'
                    )
                    break
                }
