describe('Filtros - Teste de filtragens', () => {
    ...
    it('Título e subtítulos após definição de filtragem', function () {
        cy.get('h2.main-title').should(
            'have.text',
            'Cobertura vacinal para Caxumba em 1 ano de idade por estado em 2001'
        )

        cy.get('h3.sub-title').should(
            'have.text',
            'Inclui todas as 1ª doses das vacinas do calendário vacinal da criança com componente Caxumba'
        )
    })
    ...
    it('Mudança de tipo de doença', function () {
        ...
        cy.get('h2.main-title').should(
            'have.text',
            'Cobertura vacinal para Coqueluche em menores de 1 ano por estado em 2000'
        )

        cy.get('h3.sub-title').should(
            'have.text',
            'Inclui todas as 1ª doses das vacinas do calendário vacinal da criança com componente Coqueluche'
        )
    })
    ...
    it('Limpar filtros', function () {
        ...
        cy.get('[data-test="select-granularity"]').should(
            'contain',
            'Selecione'
        )
    })
